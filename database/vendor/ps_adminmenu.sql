-- HimotheeCore Stage 1E - ps-adminmenu persistence compatibility.
-- This file is deliberately redeploy-safe and uses only single-statement triggers
-- so txAdmin can execute it directly through mysql2 without DELIMITER handling.

SET NAMES utf8mb4;

-- Standard QBCore administration tables used directly by ps-adminmenu.
...
-- ps-adminmenu's Admin Car / Give Car inserts do not supply an id, matching the
-- normal QBCore schema where player_vehicles.id is AUTO_INCREMENT.
ALTER TABLE `player_vehicles`
    MODIFY COLUMN `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT;

-- Newly inserted compatibility vehicles must become native Himothee vehicles.
...
-- Replace the Stage 1E vehicle UPDATE trigger with the full admin/MDT write-back.
-- Matching on immutable native id means plate changes also persist correctly.

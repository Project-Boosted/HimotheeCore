...
local function invoke(entry)
    if type(entry.action) == 'function' then entry.action() return end
    local params = type(entry.params) == 'table' and entry.params or {}
    local event = params.event
    if not event then return end
    if params.isServer then TriggerServerEvent(event, params.args)
    elseif params.isCommand then ExecuteCommand(event)
    elseif params.isQBCommand then TriggerServerEvent('QBCore:CallCommand', event, params.args)
    elseif params.isAction and type(event) == 'function' then event(params.args)
    elseif type(event) == 'string' then TriggerEvent(event, params.args) end
end
...
local function normalise(data, sort, skipFirst)
    ...
    if sort then
        ...
        table.sort(entries, function(a, b) return tostring(a.data.header or '') < tostring(b.data.header or '') end)
...
local function openMenu(data, sort, skipFirst)
    ...
    lib.registerContext({ id = contextId, title = 'Menu', options = normalise(data, sort, skipFirst), onExit = function() TriggerEvent('qb-menu:client:menuClosed') end })
    lib.showContext(contextId)
    return true
end
local function closeMenu() lib.hideContext(false) return true end
local function showHeader(data) return openMenu(data, false, false) end
...
exports('Health', function() return true, 'qb-menu->ox_lib' end)

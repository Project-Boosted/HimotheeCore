...
local function mapProp(prop)
    ...
    return {
        model = prop.model,
        bone = tonumber(prop.bone) or 60309,
        pos = vectorTable(prop.coords) or vectorTable(prop.pos) or { x = 0.0, y = 0.0, z = 0.0 },
        rot = vectorTable(prop.rotation) or vectorTable(prop.rot) or { x = 0.0, y = 0.0, z = 0.0 }
    }
end
...
local function mapData(data)
    ...
    if animation.task then
        anim = { scenario = animation.task }
    elseif animation.animDict or animation.dict then
        anim = {
            dict = animation.animDict or animation.dict,
            clip = animation.anim or animation.clip,
            flag = tonumber(animation.flags or animation.flag)
        }
    end

    local props = {}
    local first = mapProp(data.prop)
    local second = mapProp(data.propTwo)
    if first then props[#props + 1] = first end
    if second then props[#props + 1] = second end

    return {
        duration = math.max(1, tonumber(data.duration) or 1000),
        label = tostring(data.label or ''),
        useWhileDead = data.useWhileDead == true,
        canCancel = data.canCancel ~= false,
        disable = {
            move = controls.disableMovement == true,
            car = controls.disableCarMovement == true,
            combat = controls.disableCombat == true,
            mouse = controls.disableMouse == true,
            sprint = controls.disableMovement == true
        },
        anim = anim,
        prop = #props == 0 and nil or (#props == 1 and props[1] or props)
    }
end
...
local function run(data, finish, onStart, onTick)
    ...
    if type(onTick) == 'function' then
        CreateThread(function()
            while ticking and lib.progressActive() do
                pcall(onTick)
                Wait(100)
            end
        end)

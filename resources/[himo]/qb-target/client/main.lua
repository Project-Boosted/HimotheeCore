...
local function mergeGroups(option)
    ...
    local function add(value)
        ...
        if type(value) == 'string' then groups = groups or {}; groups[value] = 0
        elseif type(value) == 'table' then
            groups = groups or {}
            if #value > 0 then for _, name in ipairs(value) do groups[name] = 0 end
            else for name, grade in pairs(value) do groups[name] = tonumber(grade) or 0 end end
        end
    end
    add(option.job); add(option.gang); return groups
end
...
local function convertOptions(targetOptions, prefix)
    ...
    for index, option in ipairs(sourceOptions) do
        ...
        if type(action) == 'function' then entry.onSelect = function(data) action(data.entity) end
        elseif option.event then
            local optionType = tostring(option.type or 'client'):lower()
            if optionType == 'server' then entry.serverEvent = option.event
            elseif optionType == 'command' then entry.command = option.event
            else entry.event = option.event end
        end
...
local function rememberZone(name, id) if name then zoneIds[tostring(name)] = id end return id end
local function removeZone(name)
    local key = tostring(name); local id = zoneIds[key] or name
    exports.ox_target:removeZone(id); zoneIds[key] = nil
end
local function addBoxZone(name, center, length, width, zoneOptions, targetOptions)
    ...
    local coords = vec3(center); local minZ, maxZ = tonumber(zoneOptions.minZ), tonumber(zoneOptions.maxZ)
    ...
    return rememberZone(name, exports.ox_target:addBoxZone({ name = tostring(name), coords = coords, size = vector3(tonumber(length) or 1.0, tonumber(width) or 1.0, height), rotation = tonumber(zoneOptions.heading or zoneOptions.rotation) or 0.0, debug = zoneOptions.debugPoly == true, options = convertOptions(targetOptions, tostring(name)) }))
end
local function addCircleZone(name, center, radius, zoneOptions, targetOptions)
    zoneOptions = type(zoneOptions) == 'table' and zoneOptions or {}
    return rememberZone(name, exports.ox_target:addSphereZone({ name = tostring(name), coords = vec3(center), radius = tonumber(radius) or 1.0, debug = zoneOptions.debugPoly == true, options = convertOptions(targetOptions, tostring(name)) }))
end
local function addPolyZone(name, points, zoneOptions, targetOptions)
    zoneOptions = type(zoneOptions) == 'table' and zoneOptions or {}; local mapped = {}
    for _, point in ipairs(type(points) == 'table' and points or {}) do mapped[#mapped + 1] = vec3(point) end
    return rememberZone(name, exports.ox_target:addPolyZone({ name = tostring(name), points = mapped, thickness = tonumber(zoneOptions.maxZ and zoneOptions.minZ and (zoneOptions.maxZ - zoneOptions.minZ)) or 4.0, debug = zoneOptions.debugPoly == true, options = convertOptions(targetOptions, tostring(name)) }))
end

local function addTargetEntity(entities, targetOptions) exports.ox_target:addLocalEntity(entities, convertOptions(targetOptions, 'entity')) end
local function removeTargetEntity(entities, labels) exports.ox_target:removeLocalEntity(entities, labels) end
local function addTargetModel(models, targetOptions) exports.ox_target:addModel(models, convertOptions(targetOptions, 'model')) end
local function removeTargetModel(models, labels) exports.ox_target:removeModel(models, labels) end
...
exports('AddBoxZone', addBoxZone); exports('AddCircleZone', addCircleZone); exports('AddPolyZone', addPolyZone); exports('RemoveZone', removeZone)
exports('AddTargetEntity', addTargetEntity); exports('RemoveTargetEntity', removeTargetEntity); exports('AddTargetModel', addTargetModel); exports('RemoveTargetModel', removeTargetModel)
exports('AddGlobalPed', addGlobalPed); exports('RemoveGlobalPed', removeGlobalPed); exports('AddGlobalVehicle', addGlobalVehicle); exports('RemoveGlobalVehicle', removeGlobalVehicle)
exports('AddGlobalObject', addGlobalObject); exports('RemoveGlobalObject', removeGlobalObject); exports('AddGlobalPlayer', addGlobalPlayer); exports('RemoveGlobalPlayer', removeGlobalPlayer)
exports('AllowTargeting', function(allow) exports.ox_target:disableTargeting(allow == false) end)
exports('Health', function()
    return GetResourceState('ox_target') == 'started', 'qb-target->ox_target'
end)

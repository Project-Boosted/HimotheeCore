...
local function getItemBySlot(source, slot) return exports.ox_inventory:GetSlot(source, tonumber(slot)) end
local function hasItem(source, items, amount)
    amount = tonumber(amount) or 1
    if type(items) == 'string' then return (exports.ox_inventory:GetItemCount(source, items) or 0) >= amount end
    if type(items) ~= 'table' then return false end
    if #items > 0 then
        for _, item in ipairs(items) do if (exports.ox_inventory:GetItemCount(source, item) or 0) < amount then return false end end
        return true
    end
...
local function getInventory(source) return exports.ox_inventory:GetInventory(source) end
...
local function clearInventory(source, filterItems) return exports.ox_inventory:ClearInventory(source, filterItems) end
local function createShop(shopData)
    ...
    exports.ox_inventory:RegisterShop(name, { name = shopData.label or shopData.name or name, inventory = shopData.slots or shopData.items or {}, locations = shopData.locations, targets = shopData.targets, groups = shopData.groups })
    return true
end
local function openInventory(source, identifier, data)
    data = type(data) == 'table' and data or {}
    identifier = tostring(identifier or '')
    if identifier == 'player' then return exports.ox_inventory:forceOpenInventory(source, 'player', tonumber(data.id or data.source)) end
    if identifier == 'shop' then return exports.ox_inventory:forceOpenInventory(source, 'shop', data.id or data.name or data.type) end
    if identifier == 'stash' then
        local stash = data.id or data.name or data.stash
        if not stash then return false end
        if data.label or data.maxweight or data.slots then exports.ox_inventory:RegisterStash(stash, data.label or stash, tonumber(data.slots) or 50, tonumber(data.maxweight or data.weight) or 120000, data.owner, data.groups, data.coords) end
        return exports.ox_inventory:forceOpenInventory(source, 'stash', { id = stash, owner = data.owner })
    end
    if identifier ~= '' then
        exports.ox_inventory:RegisterStash(identifier, data.label or identifier, tonumber(data.slots) or 50, tonumber(data.maxweight or data.weight) or 120000, data.owner, data.groups, data.coords)
...
exports('OpenInventoryById', function(source, playerId) return exports.ox_inventory:forceOpenInventory(source, 'player', tonumber(playerId)) end)
exports('GetTotalWeight', function(items)
    local total = 0
    for _, item in pairs(type(items) == 'table' and items or {}) do total = total + ((tonumber(item.weight) or 0) * (tonumber(item.count or item.amount) or 0)) end
    return total
end)
exports('Health', function(source)
    ...
    return ok and inventory ~= nil, ok and 'qb-inventory->ox_inventory' or tostring(inventory)
end)
